...
public class MkProducerAnnotationAspect {
	...
	private ProducerData connect(MkTopicProducer topicProducer) {
		...
		producerProperties.put("metadata.broker.list", this.getBorkerHosts());

		ProducerConfig config = new ProducerConfig(producerProperties);
		Producer<Object, Object> producer = new Producer<Object, Object>(config);
	...
	private void createTopic(MkTopicProducer topicProducer) {
		...
		int repliactionFactor = topicProducer.replicationFactor();
		ZkClient zkClient = ZkClientContainer.getZkClient();
		AdminUtils.createTopic(zkClient, topic, partitions, repliactionFactor, new Properties());
	}
